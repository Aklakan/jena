    ...
    implements UpdateExec
{
    ...
    public UpdateExecOverRDFLink(Creator<RDFLink> linkCreator, Binding initialBinding, Map<Var, Node> substitutionMap, Context context,
        ...
        try {
            synchronized (cancelLock) {
                ...
                if (initialBinding != null) {
                    if (r instanceof UpdateExecDatasetBuilder db) {
                        r = db.initialBinding(initialBinding);
                    } else {
                        throw new UnsupportedOperationException("Cannot apply initial binding to delegate.");
                    }
                }
