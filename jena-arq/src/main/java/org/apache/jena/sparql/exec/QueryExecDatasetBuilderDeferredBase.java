...
{
    @Override
    public QueryExecDatasetBuilderDeferredBase<X> dataset(DatasetGraph dsg) {
        this.dataset = dsg;
        return thisBuilder();
    }
    ...
    protected QueryExecBuilder applySettings(QueryExecBuilder dest) {
        ...
        if(true) {
            ...
            // TODO Can be removed.
            Context baseCxt = contextAcc.getExplicitBaseContext();
            if (baseCxt != null) {
                dest = dest.context(baseCxt);
            }

            Context addedCxt = contextAcc.getAddedContext();
            QueryExecBuilder[] ref = {dest};
            addedCxt.forEach((k, v) -> ref[0] = ref[0].set(k, v));
            dest = ref[0];
        }

        if (initialBinding != null) {
            if (dest instanceof QueryExecDatasetBuilder dsBuilder) {
                dest = dsBuilder.initialBinding(initialBinding);
            } else {
                throw new UnsupportedOperationException("The target builder does not support initial bindings.");
            }
        }
