 ...
 * Blank nodes should be avoided because they are likely to become renamed across separate requests.
 *
 * Invocation of deleteAny() across the default graph and all named graphs fires two requests.
 * All other methods fire a single request.
 */
// FIXME: Make UpdateSink configurable without subclassing.
    ...
    extends DatasetGraphBase
{
     ...
     * Alternative implementations could e.g. flush by the string length of the update request.
     */
    protected StreamRDF newUpdateSink() {
        StreamRDF sink = new StreamRDFToUpdateRequest(this::execUpdate, Prefixes.adapt(getPrefixes()), 1000);
        return sink;
    }
