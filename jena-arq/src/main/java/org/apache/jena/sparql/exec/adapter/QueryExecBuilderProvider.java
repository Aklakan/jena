...
public interface QueryExecBuilderProvider {
    /**
     * Tests whether {@link #create(DatasetGraph, Context)} will return an
     * appropriate query execution builder for the given dataset graph.
     */
    boolean accept(DatasetGraph dsg, Context context);

    /**
     * Create a new query execution builder for the given dataset graph.
     * <p>
     * This method must only be called with arguments for which
     * {@link #accept(DatasetGraph, Context)} returns true.
     */
    QueryExecBuilder create(DatasetGraph dsg, Context context);
}
