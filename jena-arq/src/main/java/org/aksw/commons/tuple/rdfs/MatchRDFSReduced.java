    ...
    extends TupleFinder3Wrapper<D, C, TupleFinder3<D, C>>
{
    ...
    public class Worker_S_ANY_ANY {
        ...
        protected Cache<C, Set<C>> seenTypesCache = Caffeine.newBuilder().maximumSize(10_000).build();
        protected Cache<C, Set<C>> seenOutPredicatesCache = Caffeine.newBuilder().maximumSize(10_000).build();
        protected Cache<Tuple2<C>, Set<C>> seenLinksCache = Caffeine.newBuilder().maximumSize(100_000).build();
        ...
        public Stream<D> find() {
            ...
            return backend.find(ms, mp , mo).flatMap(tuple -> Iter.asStream(this.inf(tuple)));
        }
        ...
        protected Iterator<D> withTypeInfs(Iterator<D> result, C s, Set<C> newTypes) {
            if (newTypes != null) {
                result = getOrConcat(result, Iter.iter(newTypes).map(t -> tuple(s, cxtInf.rdfType, t)));
            }
            return result;
        }
