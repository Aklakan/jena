...
public class BenchmarkGeoQueries {
    ...
    public void setupTrial() throws Exception {
        ...
        // Make the query geom smaller
        Geometry envGeom = CustomGeometryFactory.theInstance().toGeometry(envelope);
        double x = envGeom.getCentroid().getX();
        double y = envGeom.getCentroid().getY();
        envGeom = AffineTransformation.scaleInstance(0.25, 0.25, x, y).transform(envGeom);

        Node wktNode = toWrapperWkt(envGeom).asNode();
        graph.add(featureNode, Geo.HAS_GEOMETRY_NODE, geometryNode);
        graph.add(geometryNode, Geo.AS_WKT_NODE, wktNode);
        ...
        case "current" -> new GeoQueryTaskCurrent();
        case "5.5.0" -> new GeoQueryTask550();
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = getDefaults(BenchmarkGeoQueries.class).build();
        new Runner(opt).run();
    }
}
