 ...
 * </ul>
 */
class LeapFrogJoinIteratorOptimized extends QueryIter {
   ...
   @Override
    ...
    protected Binding moveToNextBinding() {
        Binding result = SolverLibTDB.convToBinding(slot, nodeTable);
        slot = null;
        if (finishedAfterCurrent) {
            finished = true;
        }
        ...
        while (!finished) {
            ...
            if (allAligned) {
                 ...
                 if (mergedResult != null) {
                     ...
                     BindingNodeId savedSlot = SolverLibTDB.convert(mergedResult, nodeTable);
                     int advancedCount = 0;
    ...
    private BindingNodeId peek(QueryIterPeek it) {
        ...
        BindingNodeId result = SolverLibTDB.convert(raw, nodeTable);
        return result;
    }
    ...
    private Binding tryMergeBindings() {
        ...
        Binding result = SolverLibTDB.convToBinding(firstBinding, nodeTable);
        ...
        for (int i = 1; i < iteratorStates.size(); i++) {
            ...
            Binding converted = SolverLibTDB.convToBinding(current, nodeTable);
