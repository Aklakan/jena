...
public class SpatialObjectGeometryLiteral {
     ...
     */
    // XXX This should return an iterator over all geometry literals rather than just picking a random one.
    protected static final SpatialObjectGeometryLiteral retrieve(Graph graph, Node targetSpatialObject) {
        ...
        Node literalNode = AccessGeoSparql.getGeometry(graph, geometry);

        // Last resort: Try the legacy WGS84 Geo Positioning vocabulary on the targetSpatialObject.
        if (literalNode == null) {
            literalNode = AccessWgs84.getGeometry(graph, targetSpatialObject);
        }
